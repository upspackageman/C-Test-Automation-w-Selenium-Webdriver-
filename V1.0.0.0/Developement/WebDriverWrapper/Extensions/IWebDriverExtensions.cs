...
namespace WebDriverWrapper.Extensions
{
    public static class IWebDriverExtensions
    {
        ...
        public static void SubmitForm(this IWebDriver driver, string formId)
        {
            try
            {
                ((IJavaScriptExecutor)driver).ExecuteScript(string.Format("document.forms[\"{0}\"]", formId));
            }
            catch (Exception)
            {
        ...
        public static void SubmitForm(this IWebDriver driver, int formIndex)
        {
            try
            {
                ((IJavaScriptExecutor)driver).ExecuteScript(string.Format("document.forms[{0}].submit()", formIndex.ToString()));
            }
            catch (Exception)
            {
